...
\newcommand{\TM}{\textsc{Trajectory Mosaicing}\xspace}

\title{Trajectory Mosaicing}

\author{Eli Packer%\inst{1}
\and
Sergey Pupyrev%\inst{2}
\and
Alon Efrat%\inst{2}
\and
Stephen G. Kobourov%\inst{2}
}

%\institute{
%  IBM Research, University of Haifa Campus, Israel
%\and
...
Trajectories are arguably the most interesting pieces of information created by location services.
...
The availability of trajectory data offer unique opportunities to gain insight. For example, analyzing the complicated relationships among trajectories can help alleviate traffic congestion and thus save time and money, while decreasing pollution.
...
extensively investigated in recent years. In
...
\label{sec:ant}
Our motivation comes from biologists who wish to discover longitudinal
behavioral patterns in ant colonies. The trajectories of individual ants in a colony extracted from videos are
needed to answer questions such as how often do ants communicate, what different roles do ants play in
a colony, and how does interaction and communication affect the success or failure of a colony? 

On one hand, existing automated solutions for extracting trajectories are not good enough, and there 
is only so much data that even motivated students can annotate in the research lab.
On the other hand, in the last few years it has been shown that citizen scientists can contribute to
image processing tasks, such as the Galaxy Zoo project in which thousands of citizen scientists labeled
millions of images of galaxies from the Hubble Deep Sky Survey, 
and FoldIt in which online gamers
helped to decode the structure of an AIDS protein~--- a problem which stumped researchers for 15 years.
Such an approach has been recently applied for
extracting accurate ant trajectories from many (inaccurate) input
trajectories contributed by citizen scientists~\cite{ants}. 
In this setting, volunteers 
(citizen scientists) interact with
an online game showing a video of an ant colony; the goal is
to generate a trajectory of a specified ant via mouse clicks.
Unfortunately, the method suggested in \cite{ants} has several limitations.
First, volunteers may make mistakes. Beyond innocent mistakes, there are cases of malicious or purposeful mis-annotation. Hence, it is desirable to collect several user trajectories of the same and then compute
a consensus (``average'') trajectory.
Second, the method of relies on the assumption that all the contributed trajectories
span the entire video. However in practice citizen scientists are able to
track ants only during short video segments. Thus, in order to
reconstruct a complete ant trajectory, one needs to stitch together
short pieces of overlapping trajectories; see Figure~\ref{fig:stitch}.

In this paper, we extend and make practical the citizen science approach presented in \cite{ants}.
Let a {\em trajectory} be defined as a polyline in $\reals^2$, represented by
a sequence $p_1, \dots, p_m$ of $m$ points. 
\todo{make $m$ different for diff ants}
In addition to the spatial
coordinates, temporal information is also available.
Each point $p_i=(x_i,y_i)$ is associated with a timestamp $0 \le t_i \le 1$ and every
trajectory is monotone in time: for each $1 < i < j < T$, it holds $t_1 = 0 < t_i < t_j < t_T = 1$. (Hence, a
trajectory can be viewed as a monotone (with respect to time) polyline in
$\reals^3$ with the coordinates $x$, $y$ and $t$).
Formally, we consider the following problem, which we call \TM:
Given a set of partially overlapping trajectories, the goal is to identify complete trajectories
corresponding to every ant; see Figure~\ref{fig:stitch}.
\todo{make the problem ant-independent}
Notice the difficulty here: as the trajectories come from our online game, a correspondence between
ants and their (partial) trajectories is not part of the input.
...
\begin{enumerate}
  ...
  \item {\bf Computing a common route.} After the trajectories have been clustered,
  ...
  time. We do this by matching the common routes on the overlapping parts.
\end{enumerate}

Out algorithms work in two setting. We also need two tools: distance computation and LSH.
\todo{complete it}
...
trajectories \todo{give examples}.
...
trajectories, but due to its nature it may skip some pairs (see Section~\ref{SSS} for more details).\todo{Say why LSH skips in its section}. Augmentation of the well-known R-tree algorithm helps us further disqualify far trajectories that were skipped by LSH.

There are many applications that can benefit from a speed up
heuristic of this type. \todo{examples} In some of these applications the trajectory of the same
...
\Frechet{} distance \todo{cite}. The \Frechet{} distance is computed for two trajectories,
...
\label{sec:dist}
To measure the distance between two trajectories, we chose a metric that relates to the maximum distances along them. We distinguish among three cases, each relates to the availability of
...
\paragraph{No Time Associated Setting (NTAS)}
\label{sec:ntas}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.35\linewidth]{pics/conf_space2_with_path.jpg}
...
Note that the above algorithm is not asymptotically optimal; Agarwal~et~al.~\cite{agarwal2012computing} described a more efficient algorithm.
...
\paragraph{Time Associated Setting (TAS)}
\label{sec:tas}
...
\paragraph{Partial Time Associated Setting (PTAS)}
\label{sec:ptas}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.5\linewidth]{pics/forb_rect2.jpg}
...
\label{sec:locality}
\todo{extend it to other settings}
...
\section{Trajectory Mosaicing}
\label{sec:alg}

\subsection{Trajectory Clustering}
\label{sec:clus}

\subsection{Computing a Common Route}
\label{sec:common}
...
\todo{Need to say something about the two possible matching algorithms we used.}
\todo{need an experimental evidence that the approach helps.}

In this section we describe how to match the trajectories from both
sets \todo{Rephrase to clarify what those sets are} based on their similarity.
...
%\todo{bottleneck matching, as defined, produces poor results; need to compute
...
Note that the loop in the algorithm is responsible
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.24\linewidth]{pics/sq1_with_path.jpg}
	\includegraphics[width=0.24\linewidth]{pics/sq3_with_path.jpg}
	\includegraphics[width=0.24\linewidth]{pics/sq5_with_path.jpg}
	\includegraphics[width=0.24\linewidth]{pics/sq4_with_path.jpg}
...
\begin{figure}[h!] 	
	\centering
	\includegraphics[width=0.45\linewidth]{pics/helTrajs}
~~~
	\includegraphics[width=0.45\linewidth]{pics/roterdamTrajs}
...
\begin{figure}[h!] 	
	\centering
	\includegraphics[width=0.4\linewidth]{pics/squareTrajs.png} 	
...
\label{sec:conc}
\todo{discuss further motivations and applications}
